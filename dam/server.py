...
from MSSF.cmds.init_funcs import check_keys, MSSF_full_version_str, MSSF_init
from MSSF.cmds.passphrase_funcs import default_passphrase, using_default_passphrase
from MSSF.consensus.coinbase import create_puzzlehash_for_pk
from MSSF.daemon.keychain_server import KeychainServer, keychain_commands
from MSSF.daemon.windows_signal import kill
from MSSF.plotters.plotters import get_available_plotters
from MSSF.plotting.util import add_plot_directory
from MSSF.server.server import ssl_context_for_server
from MSSF.util.bech32m import encode_puzzle_hash
from MSSF.util.MSSF_logging import initialize_service_logging
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting MSSF.")
    quit()


log = logging.getLogger(__name__)

service_plotter = "MSSF_plotter"
...
if getattr(sys, "frozen", False):
    name_map = {
        "MSSF": "MSSF",
        "MSSF_data_layer": "start_data_layer",
        "MSSF_data_layer_http": "start_data_layer_http",
        "MSSF_wallet": "start_wallet",
        "MSSF_full_node": "start_full_node",
        "MSSF_harvester": "start_harvester",
        "MSSF_farmer": "start_farmer",
        "MSSF_introducer": "start_introducer",
        "MSSF_timelord": "start_timelord",
        "MSSF_timelord_launcher": "timelord_launcher",
        "MSSF_full_node_simulator": "start_simulator",
        "MSSF_seeder": "start_seeder",
        "MSSF_crawler": "start_crawler",
    }
...
class WebSocketServer:
        ...
        if self.ssl_context.minimum_version is not ssl.TLSVersion.TLSv1_3:
            self.log.warning(
                (
                    ...
                    "A future version of MSSF will require TLS1.3."
                ),
                ssl.OPENSSL_VERSION,
            )
    ...
    def _MSSFpos_plotting_command_args(self, request: Any, ignoreCount: bool) -> List[str]:
    ...
    def _build_plotting_command_args(self, request: Any, ignoreCount: bool, index: int) -> List[str]:
        plotter: str = request.get("plotter", "MSSFpos")
        command_args: List[str] = ["MSSF", "plotters", plotter]

        if plotter == "bladebit":
            # plotter command must be either
            # 'MSSF plotters bladebit ramplot' or 'MSSF plotters bladebit diskplot'
            plot_type = request["plot_type"]
        ...
        if plotter == "MSSFpos":
            command_args.extend(self._MSSFpos_plotting_command_args(request, ignoreCount))
        elif plotter == "madmax":
    ...
    async def start_plotting(self, websocket: WebSocketResponse, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        plotter = request.get("plotter", "MSSFpos")
...
def launch_plotter(
    ...
    # we need to pass on the possibly altered MSSF_ROOT
    os.environ["MSSF_ROOT"] = str(root_path)
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service {service_array} with MSSF_ROOT: {os.environ['MSSF_ROOT']}")
...
async def async_run_daemon(root_path: Path, wait_for_unlock: bool = False) -> int:
    # When wait_for_unlock is true, we want to skip the check_keys() call in MSSF_init
    # since it might be necessary to wait for the GUI to unlock the keyring first.
    MSSF_init(root_path, should_check_keys=(not wait_for_unlock))
    config = load_config(root_path, "config.yaml")
    setproctitle("MSSF_daemon")
    ...
    try:
        with Lockfile.create(daemon_launch_lock_path(root_path), timeout=1):
            log.info(f"MSSF-blockchain version: {MSSF_full_version_str()}")

            beta_metrics = None
            if config.get("beta", {}).get("enabled", False):
                from MSSF.util.beta_metrics import BetaMetricsLogger
