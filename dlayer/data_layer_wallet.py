...
from MSSF_rs import G1Element, G2Element
from clvm.EvalError import EvalError
from typing_extensions import Unpack, final

from MSSF.consensus.block_record import BlockRecord
from MSSF.data_layer.data_layer_errors import LauncherCoinNotFoundError, OfferIntegrityError
from MSSF.data_layer.data_layer_util import OfferStore, ProofOfInclusion, ProofOfInclusionLayer, StoreProofs, leaf_hash
from MSSF.protocols.wallet_protocol import CoinState
...
from MSSF.types.condition_opcodes import ConditionOpcode
...
from MSSF.wallet.conditions import (
    AssertAnnouncement,
...
from MSSF.wallet.db_wallet.db_wallet_puzzles import (
    ACS_MU,
    ACS_MU_PH,
    GRAFTROOT_DL_OFFERS,
    SINGLETON_LAUNCHER,
...
from MSSF.wallet.derivation_record import DerivationRecord
from MSSF.wallet.lineage_proof import LineageProof
from MSSF.wallet.outer_puzzles import AssetType
from MSSF.wallet.payment import Payment
from MSSF.wallet.puzzle_drivers import PuzzleInfo, Solver
from MSSF.wallet.puzzles.singleton_top_layer_v1_1 import SINGLETON_LAUNCHER_HASH
from MSSF.wallet.trading.offer import NotarizedPayment, Offer
from MSSF.wallet.transaction_record import TransactionRecord
from MSSF.wallet.util.compute_memos import compute_memos
from MSSF.wallet.util.merkle_utils import _simplify_merkle_proof
from MSSF.wallet.util.transaction_type import TransactionType
from MSSF.wallet.util.tx_config import CoinSelectionConfig, TXConfig, TXConfigLoader
from MSSF.wallet.util.wallet_sync_utils import fetch_coin_spend, fetch_coin_spend_for_coin_state
...
from MSSF.wallet.wallet_protocol import GSTOptionalArgs, WalletProtocol
...
class DataLayerWallet:
    ...
    async def get_launcher_coin_state(self, launcher_id: bytes32, peer: WSMSSFConnection) -> CoinState:
    ...
    async def create_tandem_xch_tx(
        ...
        [MSSF_tx] = await self.standard_wallet.generate_signed_transaction(
            amount=uint64(0),
        ...
        assert MSSF_tx.spend_bundle is not None
        return MSSF_tx
    ...
    async def create_update_state_spend(
        ...
        if fee > 0:
            MSSF_tx = await self.create_tandem_xch_tx(
                fee, AssertAnnouncement(True, asserted_origin_id=current_coin.name(), asserted_msg=b"$"), tx_config
            )
            assert MSSF_tx.spend_bundle is not None
            aggregate_bundle = SpendBundle.aggregate([dl_tx.spend_bundle, MSSF_tx.spend_bundle])
            dl_tx = dataclasses.replace(dl_tx, spend_bundle=aggregate_bundle)
            MSSF_tx = dataclasses.replace(MSSF_tx, spend_bundle=None)
            txs: List[TransactionRecord] = [dl_tx, MSSF_tx]
        else:
            txs = [dl_tx]
    ...
    async def delete_mirror(
        ...
        if excess_fee > 0:
            [MSSF_tx] = await self.wallet_state_manager.main_wallet.generate_signed_transaction(
                uint64(1),
                new_puzhash,
                tx_config,
            ...
            txs = [
                dataclasses.replace(
                    txs[0], spend_bundle=SpendBundle.aggregate([txs[0].spend_bundle, MSSF_tx.spend_bundle])
                ),
                dataclasses.replace(MSSF_tx, spend_bundle=None),
            ]
