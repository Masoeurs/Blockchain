...
git submodule update --init MSSF-blockchain-gui

Set-Location -Path ".\MSSF-blockchain-gui" -PassThru
...
# Other than `MSSF-blockchain-gui/package/gui`, all other packages are no longer necessary after build.
# Since these unused packages make cache unnecessarily fat, unused packages should be removed.
Write-Output "Remove unused @MSSF-network packages to make cache slim"
...
# Remove `packages/gui/node_modules/@MSSF-network` because it causes an error on later `electron-packager` command
#Remove-Item "@MSSF-network" -Recurse -Force
