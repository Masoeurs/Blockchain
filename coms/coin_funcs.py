...
from MSSF.cmds.cmds_util import CMDCoinSelectionConfigLoader, CMDTXConfigLoader, cli_confirm, get_wallet_client
from MSSF.cmds.units import units
from MSSF.cmds.wallet_funcs import get_mojo_per_unit, get_wallet_type, print_balance
...
from MSSF.util.ints import uint64, uint128
...
async def async_combine(
    *,
    ...
    async with get_wallet_client(wallet_rpc_port, fingerprint) as (wallet_client, fingerprint, config):
        ...
        final_fee = uint64(int(fee * units["MSSF"]))
        ...
        if not await wallet_client.get_synced():
            print("Wallet not synced. Please wait.")
            return
        is_xch: bool = wallet_type == WalletType.STANDARD_WALLET  # this lets us know if we are directly combining MSSF
    ...
    *,
    ...
    async with get_wallet_client(wallet_rpc_port, fingerprint) as (wallet_client, fingerprint, config):
        ...
        if not await wallet_client.get_synced():
            print("Wallet not synced. Please wait.")
            return
        is_xch: bool = wallet_type == WalletType.STANDARD_WALLET  # this lets us know if we are directly spitting MSSF
