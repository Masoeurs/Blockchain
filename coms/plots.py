...
from MSSF.plotting.util import add_plot_directory, validate_plot_size

log = logging.getLogger(__name__)


def show_plots(root_path: Path) -> None:
    from MSSF.plotting.util import get_plot_directories

    print("Directories where plots are being searched for:")
    print("Note that subdirectories must be added manually")
    print(
        "Add with 'MSSF plots add -d [dir]' and remove with"
        + " 'MSSF plots remove -d [dir]'"
        + " Scan and check plots with 'MSSF plots check'"
    )
    print()
...
def plots_cmd(ctx: click.Context) -> None:
    ...
    if not root_path.is_dir():
        raise RuntimeError("Please initialize (or migrate) your config directory with 'MSSF init'")
...
def create_cmd(
    ...
    from MSSF.plotting.util import Params
...
def check_cmd(
    ...
    from MSSF.plotting.check_plots import check_plots
...
def remove_cmd(ctx: click.Context, final_dir: str) -> None:
    from MSSF.plotting.util import remove_plot_directory
