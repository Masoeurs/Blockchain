...
from MSSF.cmds.configure import configure
from MSSF.consensus.coinbase import create_puzzlehash_for_pk
from MSSF.ssl.create_ssl import create_all_ssl
from MSSF.util.bech32m import encode_puzzle_hash
from MSSF.util.config import (
    create_default_MSSF_config,
    initial_config_file,
    load_config,
...
def check_keys(new_root: Path, keychain: Optional[Keychain] = None) -> None:
    ...
    if len(all_sks) == 0:
        print("No keys are present in the keychain. Generate them with 'MSSF keys generate'")
        return None
...
def init(
    ...
    if create_certs is not None:
        ...
        return MSSF_init(root_path, fix_ssl_permissions=fix_ssl_permissions, testnet=testnet, v1_db=v1_db)

    return None


def MSSF_version_number() -> Tuple[str, str, str, str]:
...
def MSSF_full_version_str() -> str:
    major, minor, patch, dev = MSSF_version_number()
...
def MSSF_init(
    ...
    *,
    ...
    """
    MSSF_root = os.environ.get("MSSF_ROOT", None)
    if MSSF_root is not None:
        print(f"MSSF_ROOT is set to {MSSF_root}")

    print(f"MSSF directory {root_path}")
    if root_path.is_dir() and Path(root_path / "config" / "config.yaml").exists():
        # This is reached if MSSF_ROOT is set, or if user has run MSSF init twice
        # before a new update.
        if testnet:
            configure(
                root_path,
    ...
    create_default_MSSF_config(root_path)
    if testnet:
        configure(
            root_path,
            set_farmer_peer="",
    ...
    print("")
    print("To see your keys, run 'MSSF keys show --show-mnemonic-seed'")
