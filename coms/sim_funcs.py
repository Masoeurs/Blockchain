...
from MSSF_rs import PrivateKey

from MSSF.cmds.cmds_util import get_any_service_client
from MSSF.cmds.start_funcs import async_start
from MSSF.consensus.coinbase import create_puzzlehash_for_pk
from MSSF.server.outbound_message import NodeType
from MSSF.simulator.simulator_full_node_rpc_client import SimulatorFullNodeRpcClient
from MSSF.types.blockchain_format.sized_bytes import bytes32
from MSSF.types.coin_record import CoinRecord
from MSSF.util.bech32m import decode_puzzle_hash, encode_puzzle_hash
from MSSF.util.config import load_config, save_config, set_peer_info
from MSSF.util.errors import KeychainFingerprintExists
from MSSF.util.ints import uint32
from MSSF.util.keychain import Keychain, bytes_to_mnemonic
...
def create_MSSF_directory(
    ...
    """
    This function creates a new MSSF directory and returns a heavily modified config,
    suitable for use in the simulator.
    """
    from MSSF.cmds.init_funcs import MSSF_init

    if not MSSF_root.is_dir() or not Path(MSSF_root / "config" / "config.yaml").exists():
        # create MSSF directories & load config
        MSSF_init(MSSF_root, testnet=True, fix_ssl_permissions=True)
        config: Dict[str, Any] = load_config(MSSF_root, "config.yaml")
        ...
        config = load_config(MSSF_root, "config.yaml")
    # simulator overrides
    ...
    save_config(MSSF_root, "config.yaml", config)
    return config


def display_key_info(fingerprint: int, prefix: str) -> None:
    """
    Display key info for a given fingerprint, similar to the output of `MSSF keys show`.
    """
...
def generate_and_return_fingerprint(mnemonic: Optional[str] = None) -> int:
    ...
    """
    from MSSF.util.keychain import generate_mnemonic
...
async def generate_plots(config: Dict[str, Any], root_path: Path, fingerprint: int, bitfield: bool) -> None:
    ...
    from MSSF.simulator.block_tools import BlockTools, test_constants
    from MSSF.simulator.start_simulator import PLOT_SIZE, PLOTS

    farming_puzzle_hash = decode_puzzle_hash(config["simulator"]["farming_address"])
    os.environ["MSSF_ROOT"] = str(root_path)  # change env variable, to make it match what the daemon would set it to
...
async def async_config_wizard(
    ...
    if fingerprint is None:
        ...
        return
    # create MSSF directory & get config.
    print("Creating MSSF directory & config...")
    config = create_MSSF_directory(root_path, fingerprint, farming_address, plot_directory, auto_farm, docker_mode)
    ...
    sys.argv[0] = str(Path(sys.executable).parent / "MSSF")  # fix path for tests
    ...
    while True:
        try:
            ...
            break
    print(f"\nMake sure your MSSF_ROOT Environment Variable is set to: {root_path}")
...
async def print_status(
    ...
    """
    from MSSF.cmds.show_funcs import print_blockchain_state
    ...
    async with get_any_service_client(SimulatorFullNodeRpcClient, rpc_port, root_path) as (node_client, config):
        ...
        # chain status ( basically MSSF show -s)
        ...
        print(
            ...
            f"with a balance of: "
            f"{sum(coin_records.coin.amount for coin_records in farming_coin_records) / units['MSSF']} TXCH."
        )
        if show_addresses:
            print("All Addresses: ")
