...
from MSSF.cmds.wallet_funcs import print_balance, wallet_coin_unit
from MSSF.pools.pool_config import PoolWalletConfig, load_pool_config, update_pool_config
from MSSF.pools.pool_wallet_info import PoolSingletonState, PoolWalletInfo
...
async def create(
    ...
    async with get_wallet_client(wallet_rpc_port, fingerprint) as (wallet_client, fingerprint, _):
        fee_mojos = uint64(int(fee * units["MSSF"]))
        ...
        if user_input.lower() == "y" or user_input.lower() == "yes":
            try:
                ...
                print(
                    f"Error creating plot NFT: {e}\n    Please start both farmer and wallet with:  MSSF start -r farmer"
                )
            return
        print("Aborting.")
