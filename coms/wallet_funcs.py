...
from MSSF.cmds.cmds_util import (
    CMDTXConfigLoader,
    cli_confirm,
    get_wallet_client,
...
from MSSF.cmds.peer_funcs import print_connections
from MSSF.cmds.units import units
from MSSF.rpc.wallet_request_types import GetNotifications
from MSSF.rpc.wallet_rpc_client import WalletRpcClient
from MSSF.types.blockchain_format.sized_bytes import bytes32
from MSSF.util.bech32m import bech32_decode, decode_puzzle_hash, encode_puzzle_hash
from MSSF.util.byte_types import hexstr_to_bytes
from MSSF.util.config import selected_network_address_prefix
from MSSF.util.ints import uint16, uint32, uint64
from MSSF.wallet.conditions import CreateCoinAnnouncement, CreatePuzzleAnnouncement
from MSSF.wallet.nft_wallet.nft_info import NFTInfo
from MSSF.wallet.outer_puzzles import AssetType
from MSSF.wallet.puzzle_drivers import PuzzleInfo
from MSSF.wallet.trade_record import TradeRecord
from MSSF.wallet.trading.offer import Offer
from MSSF.wallet.trading.trade_status import TradeStatus
from MSSF.wallet.transaction_record import TransactionRecord
from MSSF.wallet.transaction_sorting import SortKey
from MSSF.wallet.util.address_type import AddressType, ensure_valid_address
from MSSF.wallet.util.puzzle_decorator_type import PuzzleDecoratorType
from MSSF.wallet.util.query_filter import HashFilter, TransactionTypeFilter
from MSSF.wallet.util.transaction_type import CLAWBACK_INCOMING_TRANSACTION_TYPES, TransactionType
from MSSF.wallet.util.wallet_types import WalletType
from MSSF.wallet.vc_wallet.vc_store import VCProofs
from MSSF.wallet.wallet_coin_store import GetCoinRecords
...
def print_transaction(
    ...
    if verbose:
        ...
        MSSF_amount = Decimal(int(tx.amount)) / mojo_per_unit
        ...
        print(f"Amount {description}: {MSSF_amount} {name}")
...
async def send(
    *,
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        ...
        final_fee: uint64 = uint64(int(fee * units["MSSF"]))  # fees are always in XCH mojos
        ...
        print(f"To get status, use command: MSSF wallet get_transaction -f {fingerprint} -tx 0x{tx_id}")
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        ...
        if offers == [] or requests == []:
            ...
            for item in [*offers, *requests]:
                ...
                if fee > 0:
                    print()
                    print(f"Including Fees: {Decimal(fee) / units['MSSF']} XCH, {fee} mojos")
                ...
                if royalty_asset_dict != {}:
                    ...
                    for nft_id, summaries in royalty_summary.items():
                        ...
                        for summary in summaries:
                            divisor = units["MSSF"] if summary["asset"] == "XCH" else units["cat"]
                    ...
                    for asset, requested_amount in total_amounts_requested.items():
                        divisor = units["MSSF"] if asset == "XCH" else units["cat"]
                ...
                with filepath.open(mode="w") as file:
                    offer, trade_record = await wallet_client.create_offer_for_ids(
                        ...
                        tx_config=CMDTXConfigLoader(
                            reuse_puzhash=reuse_puzhash,
                        ).to_tx_config(units["MSSF"], config, fingerprint),
                    )
                    if offer is not None:
                        file.write(offer.to_bech32())
                        print(f"Created offer with ID {trade_record.trade_id}")
                        print(
                            f"Use MSSF wallet get_offers --id "
...
async def print_trade_record(record: TradeRecord, wallet_client: WalletRpcClient, summaries: bool = False) -> None:
    ...
    if summaries:
        ...
        print(f"Included Fees: {fees / units['MSSF']} XCH, {fees} mojos")
    print("---------------")
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        ...
        if royalty_asset_dict != {}:
            ...
            if fungible_asset_dict != {}:
                ...
                for nft_id, summaries in royalty_summary.items():
                    print(f"  - For {nft_id}:")
                    for summary in summaries:
                        divisor = units["MSSF"] if summary["asset"] == network_xch else units["cat"]
                        converted_amount = Decimal(summary["amount"]) / divisor
                        total_amounts_requested.setdefault(summary["asset"], fungible_asset_dict[summary["asset"]])
                        total_amounts_requested[summary["asset"]] += summary["amount"]
                        print(
                ...
                for asset, amount in total_amounts_requested.items():
                    divisor = units["MSSF"] if asset == network_xch else units["cat"]
                    converted_amount = Decimal(amount) / divisor
                    print(f"  - {converted_amount} {asset} ({amount} mojos)")

        print(f"Included Fees: {Decimal(offer.fees()) / units['MSSF']} {network_xch}, {offer.fees()} mojos")
        ...
        if not examine_only:
            ...
            trade_record = await wallet_client.take_offer(
                offer,
                fee=fee,
                tx_config=CMDTXConfigLoader().to_tx_config(units["MSSF"], config, fingerprint),
            )
            print(f"Accepted offer with ID {trade_record.trade_id}")
            print(f"Use MSSF wallet get_offers --id {trade_record.trade_id} -f {fingerprint} to view its status")
...
async def cancel_offer(
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        ...
        await wallet_client.cancel_offer(
            offer_id, CMDTXConfigLoader().to_tx_config(units["MSSF"], config, fingerprint), secure=secure, fee=fee
        )
        print(f"Cancelled offer with ID {trade_record.trade_id}")
        if secure:
            print(f"Use MSSF wallet get_offers --id {trade_record.trade_id} -f {fingerprint} to view cancel status")
...
def wallet_coin_unit(typ: WalletType, address_prefix: str) -> Tuple[str, int]:  # pragma: no cover
    ...
    if typ in [WalletType.STANDARD_WALLET, WalletType.POOLING_WALLET, WalletType.MULTI_SIG]:
        return address_prefix, units["MSSF"]
    return "", units["mojo"]
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        try:
            response = await wallet_client.did_message_spend(
                did_wallet_id,
                CMDTXConfigLoader().to_tx_config(units["MSSF"], config, fingerprint),
                extra_conditions=(
...
async def send_notification(
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        ...
        amount: uint64 = uint64(d_amount * units["MSSF"])
        message: bytes = bytes(message_hex, "utf8")
        fee: uint64 = uint64(d_fee * units["MSSF"])

        tx = await wallet_client.send_notification(address, message, amount, fee)

        print("Notification sent successfully.")
        print(f"To get status, use command: MSSF wallet get_transaction -f {fingerprint} -tx 0x{tx.name}")
    ...
    *, wallet_rpc_port: Optional[int], fp: Optional[int], fee: Decimal, tx_ids_str: str, force: bool = False
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, _, _):
        ...
        if fee < 0:
            ...
            return
        response = await wallet_client.spend_clawback_coins(tx_ids, int(fee * units["MSSF"]), force)
        print(str(response))
    ...
    *,
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        txs = await wallet_client.vc_spend(
            ...
            fee=uint64(int(d_fee * units["MSSF"])),
            tx_config=CMDTXConfigLoader(
                reuse_puzhash=reuse_puzhash,
            ).to_tx_config(units["MSSF"], config, fingerprint),
        )
